...
int main()
{
    ...
    initialize_robot();
	setAngles();
        get_front_ir_dists(&startLeftIR, &startRightIR);

    printf("%d %d\n", startRightIR, startLeftIR);
    inputMotors();
    //wallFollowing();
...
void setAngles(){
    set_ir_angle(RIGHT, 30);
    set_ir_angle(LEFT, -30);
}
...
void wallFollowingR()
{
    ...
    //for(i = 0; i < 170; i++)
    {
        ...
        if (distAhead < 30){    
            ...
            { 
            //set_motors(SPEED, -SPEED);  
            set_motors(SPEED+15, SPEED-15); 
            } //turn right

            //Note was originally 20
             else if (rightFrontIR - startRightIR < 0)
            {   
            //set_motors(-SPEED, SPEED);
                set_motors (SPEED-20, SPEED+20);
             } //turn left
